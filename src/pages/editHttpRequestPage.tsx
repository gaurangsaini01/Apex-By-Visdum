...
import { editMonitor, getMonitorDetails } from "../services/operations/monitor";
...
function EditHttpRequestPage() {
    ...
    const initialValues: Settings = {
        url: monitor.url??"",
        emailNotify: monitor.email_notify,
        interval: monitor.check_interval,
        timeout: monitor.timeout ?? 15,
        statusCodes: monitor.http_incidents_code ?? [],
        authType: monitor.auth_type ?? "None",
        token: monitor.auth_token ?? "",
        httpMethod: monitor.http_method ?? "HEAD",
    ...
    const handleSubmit = async (values: Settings) => {
        ...
        await editMonitor(id, data, token, navigate)
    }
                ...
                <div className="p-4 text-dark bg-white shadow-sm rounded ">
                    ...
                    >
                        {({
                            ...
                            console.log(values.emailNotify)
                            useEffect(() => {
...
export default EditHttpRequestPage;
